...
if os.path.isfile('Wordlist.txt'):
    ...
    os.system('wget -q https://raw.githubusercontent.com/Goutham-777/million-wordlist/main/Wordlist.txt')
else:
    print ('Downloading Wordlist......')
    os.system('wget -q https://raw.githubusercontent.com/Goutham-777/million-wordlist/main/Wordlist.txt')
...
def banner():
    print ('\033[93m-'*30)
    print ('Simple HashCracker by Goutham')
    print ('\033[93m-'*30)
...
def Hash():
    ...
    get =  (colored('\n(1)md5\n(2)sha1\n(3)sha512\n(4)base64\n(5)base32\n(6)base16\n(7)Reverse cipher\n(8)blake2b\n(9)blake2s\n(10)sha224\n(11)sha384\n(12)Find Hash type\n(13)Hex\n(14)Base85\n(15)Crypt', 'cyan'))
    print (get)
    post = input('\033[91mEnter your Option: ')
    txt = ('Wordlist.txt')
    if post == '1':
        like = input('(1)Encrypt\n(2)Decrypt:')
        if like == '1':
              hex = input(colored('Your word to Encrypt: ', 'blue'))
              g = hashlib.md5(hex.encode().strip()).hexdigest()
              print (g)
        elif like == '2':
            de = input(colored('Enter your hash to decrypt: ', 'blue'))
            passwd = open(txt, 'r+')
            for i in passwd:
                j = i.encode('utf-8')
                ga = hashlib.md5(j.strip()).hexdigest()
                if ga == de:
                    print (colored(f'\nHash: {i}', 'green'))
                    sys.exit()
                else:
        ...
        like = input('\033[93m(1)Encrypt\n(2)Decrypt: ')
        if like == '1':
             hex = input(colored('Your word to Encrypt: ', 'blue'))
             g = hashlib.sha1(hex.encode().strip()).hexdigest()
             print (g)
        elif like == '2':
             de = input(colored('Enter your hash to decrypt: ', 'blue'))
             passwd = open(txt, 'r+')
             for i in passwd:
                j = i.encode('utf-8')
                ga = hashlib.sha1(j.strip()).hexdigest()
                if ga == de:
                    print (colored(f'\nHash: {i}', 'green'))
                    sys.exit()
                else:
        ...
        if like == '1':
             hex = input(colored('Your word to Encrypt: ', 'blue'))
             g = hashlib.sha512(hex.encode().strip()).hexdigest()
             print (g)
        elif like == '2':
            de = input(colored('Enter your hash to decrypt: ', 'blue'))
            passwd = open(txt, 'r+')
            for i in passwd:
                j = i.encode('utf-8')
                ga = hashlib.sha512(j.strip()).hexdigest()
                if ga == de:
                    print (colored(f'\nHash: {i}', 'green'))
                    sys.exit()
                else:
         ...
         base = input('(1)Encrypt\n(2)Decrypt: ')
         if base == '1':
             words = input(colored("Enter your word to Encrypt: ", 'blue'))
             word = words.encode()
             id = base64.b64encode(word)
             print (id.decode())
         elif base == ('2'):
             mes = input(colored('Enter your hash decrypt: ', 'blue'))
             me = mes.encode()
             gs = base64.b64decode(me)
             print (colored(f'Hash: {gs.decode()}', 'green'))
        ...
        if base == '1':
            words = input(colored("Enter your word to Encrypt: ", 'blue'))
            word = words.encode()
            id = base64.b32encode(word)
            print (id.decode('utf-8'))
        elif base == '2':
            mes = input(colored('Enter your hash decrypt: ', 'blue'))
            me = mes.encode()
            gs = base64.b32decode(me)
            print (colored(f'Hash: {gs.decode()}', 'green'))
        else:
            sys.exit()
    elif post == '6':
        ...
        if base == '1':
             words = input(colored("Enter your word to Encrypt: ", 'blue'))
             word = words.encode()
             id = base64.b16encode(word)
             print (id.decode())
        elif base == ('2'):
             mes = input(colored('Enter your hash decrypt: ', 'blue'))
             me = mes.encode()
             gs = base64.b16decode(me)
             print (colored(f'Hash: {gs.decode()}', 'green'))

    elif post == '7':
        mes = input('Enter your word Encrypt/Decrypt: ')
        trans = ''
        x = len(mes) - 1
        while x >= 0:
            trans = trans + mes[x]
            x = x -1
        print (colored(f'Hash: {trans}', 'green'))

    elif post == '8':
        like = input('\033[93m(1)Encrypt\n(2)Decrypt: ')
        if like == '1':
             hex = input(colored('Your word to Encrypt: ', 'blue'))
             g = hashlib.blake2b(hex.encode().strip()).hexdigest()
             print (g)
        elif like == '2':
             de = input(colored('Enter your hash to decrypt: ', 'blue'))
             passwd = open(txt, 'r+')
             for i in passwd:
                j = i.encode('utf-8')
                ga = hashlib.blake2b(j.strip()).hexdigest()
                if ga == de:
                    print (colored(f'\nHash: {i}', 'green'))
                    sys.exit()
                else:
        ...
        if like == '1':
             hex = input(colored('Your word to Encrypt: ', 'blue'))
             g = hashlib.blake2s(hex.encode().strip()).hexdigest()
             print (g)
        elif like == '2':
             de = input(colored('Enter your hash to decrypt: ', 'blue'))
             passwd = open(txt, 'r+')
             for i in passwd:
                j = i.encode('utf-8')
                ga = hashlib.blake2s(j.strip()).hexdigest()
                if ga == de:
                    print (colored(f'\nHash: {i}', 'green'))
                    sys.exit()
                else:
        ...
        if like == '1':
             hex = input(colored('Your word to Encrypt: ', 'blue'))
             g = hashlib.sha224(hex.encode().strip()).hexdigest()
             print (g)
        elif like == '2':
             de = input(colored('Enter your hash to decrypt: ', 'blue'))
             passwd = open(txt, 'r+')
             for i in passwd:
                j = i.encode('utf-8')
                ga = hashlib.sha224(j.strip()).hexdigest()
                if ga == de:
                    print (colored(f'\nHash: {i}', 'green'))
                    sys.exit()
                else:
        ...
        if like == '1':
             hex = input(colored('Your word to Encrypt: ', 'blue'))
             g = hashlib.sha384(hex.encode().strip()).hexdigest()
             print (g)
        elif like == '2':
             de = input(colored('Enter your hash to decrypt: ', 'blue'))
             passwd = open(txt, 'r+')
             for i in passwd:
                j = i.encode('utf-8')
                ga = hashlib.sha384(j.strip()).hexdigest()
                if ga == de:
                    print (colored(f'\nHash: {i}', 'green'))
                    sys.exit()
                else:
...
while True:
    ...
    if len(jerar)==0:
        ...
        jerar.sort()
        print ""
        print "\033[92mPossible Hashs:"
        print "\033[92m[+] ",algorithms[jerar[0]]
        print "\033[92m[+] ",algorithms[jerar[1]]
        print ""
    else:
        jerar.sort()
        print ""
        ...
        for a in range(len(jerar)):
            print "\033[92m[+]",algorithms[jerar[a]]''')
            os.system('python2 hash_id.py')
            os.remove('hash_id.py')
        except:
            os.system('clear')
            os.remove('hash_id.py')

    elif post == '13':
        like = input(colored('(1)Encrypt\n(2)Decrypt: ', 'blue'))
        if like == '1':
            word = input(colored('Enter your word encrypt: ', 'red'))
            encode = binascii.hexlify(bytes(word, 'utf-8'))
            encode = str(encode).strip('b')
            encode = (encode).strip("'")
            encode = re.sub(r'(..)', r'\1 ',encode).strip()
            print (colored(f'Hash: {encode}' , 'green'))
        elif like == '2':
            word = input(colored('Enter your word decrypt: ', 'red'))
            decode = bytes.fromhex(word)
            d = base64.b64encode(decode)
            s = base64.b64decode(d)
            print (colored(f'Hash: {s}', 'green'))
    elif post == '14':
        like = input(colored('(1)Encrypt\n(2)Decrypt: ', 'blue'))
        if like == '1':
            word = input('Enter your Word to Encrypt: ')
            words = (word.encode())
            d = base64.b85encode(words)
            print ('\033[92mHash: {}'.format(d.decode()))
        elif like == '2':
            letter = input('Enter your Word to Decrypt: ')
            s = base64.b85decode(letter)
            print ('\033[92mHash: {}'.format(s.decode()))
    elif post == '15':
        print ('\033[94mOnly Encryption')
        s = input('Word to Encrypt: ')
        d = crypt.crypt(s)
        print (f'\033[92mHash: {d}')
...
if __name__ == '__main__':
        th = threading.Thread(target=Hash)
        th.start()
